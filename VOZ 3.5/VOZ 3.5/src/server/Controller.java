...
import Traoqua.ArchivementSucManh;
...
//                        byte index = _msg.reader().readByte();
                         if (player.zone != null && player.zone.map != null && player.typeRecvieArchiment == 2) {
                            ...
                            Mocnap.gI().receiveGem(_msg.reader().readByte(), player);
...
//                            player.sendNewPet();
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "|0|❤️ Server Ngọc Rồng Red Free 100% \n"
                + "• Lên web liên kết Gmail để mở thành viên\n"
                + "• Link tải game: https://dragon7super.io.vn/");
    }
