...
public abstract class Npc implements IAtionNpc {
 ...
 public void receiveReward(Player player, int nextMoc, String text, int[] baseItems, int quantity, int[]... extraItems) {
    npcChat(player, text);
    player.mocquaonline = nextMoc;
    ItemService.gI().addItemsToBag(player, baseItems, quantity);

    // Thêm các items phụ (nếu có)
    if (extraItems != null) {
        for (int[] extra : extraItems) {
            if (extra.length >= 2) {
                int itemId = extra[0];
                int itemQuantity = extra[1];
                ItemService.gI().addItemsToBag(player, new int[]{itemId}, itemQuantity);
            }
        }
    }
}
