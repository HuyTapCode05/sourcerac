...
public class Mocnap {
    ...
    public Mocnap(String info1, String info2, short money, boolean isFinish, boolean isRecieve) {
        this.info1 = info1;
        this.info2 = info2;
        this.money = money;
    ...
    public void Show(Player pl) {
        ...
        try {
            ...
            for (int i = 0; i < pl.archivementList.size(); i++) {
                ...
                Mocnap archivement = pl.archivementList.get(i);
    ...
    public boolean checkdanap(Player pl, int index) {
  ...
  public void receiveGem(int index, Player pl) {
    if (index < 0 || index >= pl.archivementList.size()) {
        Service.gI().sendThongBao(pl, "Chỉ số không hợp lệ!");
        return;
    }

    Mocnap temp = pl.archivementList.get(index);
    if (temp == null) {
    ...
    try {
        ...
        msg.writer().writeByte(index); // Index quà
        pl.sendMessage(msg);
    } catch (IOException e) {
    ...
    try {
        ...
        for (Mocnap arr : pl.archivementList) {
            dataArray.add(arr.isRecieve ? "1" : "0");
        }

        String inventory = dataArray.toJSONString();
        DBConnecter.executeUpdate("UPDATE player SET moc_nap = ? WHERE id = ?", inventory, pl.id);

        // Nhận quà
        nhanQua(pl, index + 1);
        System.out.println("Player " + pl.name + " đã nhận quà thành công!");
    } catch (Exception e) {
        e.printStackTrace();
    }
}
    ...
    public void getAchievement(Player player) {
        ...
        Connection con = DBConnecter.getConnectionServer();
         ...
         PreparedStatement ps = con.prepareStatement("SELECT `moc_nap` FROM `player` WHERE id = ? LIMIT 1");
...
//                                    achievement.setInfo1("Mốc nạp ");
                                    ...
                                    achievement.setFinish(checkdanap(player, i));
