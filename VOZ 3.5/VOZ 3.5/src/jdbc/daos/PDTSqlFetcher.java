...
public class PDTSqlFetcher {
    ...
    private static Player loadPlayer(NDVResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            dataArray.clear();
     // Time ON
                           JSONObject Timeon = (JSONObject) JSONValue.parse(rs.getString("TimeOn"));
                            player.TimeOnline = Long.parseLong(String.valueOf(Timeon.get("Timeonline")));
                            player.mocquaonline = Integer.parseInt(String.valueOf(Timeon.get("mocdanhan")));
                            player.timereset = Long.parseLong(String.valueOf(Timeon.get("timers")));
                            dataArray.clear(); 
            //data tọa độ
            try {
...
//            }
            ...
            try {
                ...
                // Gán giá trị cho thuộc tính vip
                player.vip = Integer.parseInt(String.valueOf(dataArray.get(0)));  // Giả sử phần tử đầu tiên là vip

                // Chuyển phần tử thứ hai (ngày tháng) thành LocalDate và gán cho timevip
                String timevipString = String.valueOf(dataArray.get(1));  // Phần tử thứ hai là chuỗi ngày tháng
                player.timevip = LocalDate.parse(timevipString);  // Chuyển đổi thành LocalDate
