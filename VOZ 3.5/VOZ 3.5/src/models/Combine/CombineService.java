...
import models.Combine.manifest.*;
...
public static final int CHE_TAO_SKH = 530;
    ...
    public void showInfoCombine(Player player, int[] index) {
        ...
        switch (player.combine.typeCombine) {
            ...
            case CHE_TAO_SKH ->
                CheTaoSKH.showInfoCombine(player);
...
//            msg.writer().writeShort(-1); // y
    ...
    public void sendAddItemCombine(Player player, int npcId, Item... items) {
        ...
        try {
            msg = new Message(-81);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("By VozEntertainment");
            msg.writer().writeUTF("SoulMatee - Đẳng Cấp Là Mãi Mãi");
        ...
        return switch (type) {
            ...
            case NANG_CAP_KICH_HOAT ->
                "Ta sẽ phù phép\nchế tạo trang bị Huỷ Diệt\nthành trang bị Kích Hoạt";
                 case CHE_TAO_SKH ->
                "Ta sẽ phù phép\nchế tạo thành trang bị Kích Hoạt";
            case NANG_CAP_KICH_HOAT_VIP ->
        ...
        return switch (type) {
            ...
            case NANG_CAP_KICH_HOAT ->
                "Vào hành trang\nChọn 1 trang bị Huỷ Diệt\nChọn 1 viên đá Kích Hoạt\nSau đó chọn 'Nâng cấp'";
                    case CHE_TAO_SKH ->
                "Vào hành trang\nChọn 1 trang bị Huỷ Diệt\nChọn 1000 linh hồn tương ứng \nlinh hồn hành tinh nào ra hành tinh đó"
                + "\nSau đó chọn 'Nâng cấp'";
            case NANG_CAP_KICH_HOAT_VIP ->
