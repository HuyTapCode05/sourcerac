...
public class Player implements Runnable {
    ...
    // Hệ thống quà tặng online
    public long TimeOnline;
    public long timereset;
    public int mocquaonline;
    ...
    public List<Mocnap> archivementList = new ArrayList<>();
...
//                    }
                    ...
                    if (this.isPl()) {
                        ...
                        this.TimeOnline++;
                        Logger.success("timeON!" + TimeOnline + "\n");
                        updateCSMM();
...
//        }
    ...
    public synchronized long injured(Player plAtt, long damage, boolean piercing, boolean isMobAttack) {
        Item item = inventory.itemsBody.get(10);
        if (!this.isDie()) {
...
//            }
            ...
            if (!Util.canDoWithTime(this.lastTimeRevived, 1500)) {
                return 0;
                            ...
                            List<Mob> nearbyMobs = findNearbyMobs(plAtt, 3);
                            for (Mob mob : nearbyMobs) {
                                long lanDamage = damage / 2; // Sát thương chia đôi
                                mob.injured(plAtt, lanDamage, false);
                                 Logger.error("Dame Lan"+ lanDamage +"\n");
                            }
...
//            damage = Math.min(damage, 2_000_000_000L); // Giới hạn damage không vượt quá 2 tỷ
            ...
            if ((plAtt != null || isMobAttack) && isDie() && !isBoss && !isNewPet && !isNewPet1) {
                if (item.template.id == 1785) {
                    setBom(plAtt);
                    Logger.error("Mặc Bùa Nổ\n");
                } else if (Util.isTrue(this.nPoint.tlBom, 100)) {
                    setBom(plAtt);
                    Logger.error("player die" + this.nPoint.tlBom + "\n");
                } else {
                    setDie(plAtt);
                }
            }
  ...
  private List<Mob> findNearbyMobs(Player target, int maxMobs) {
    List<Mob> mobs = new ArrayList<>();
  
    for (Mob mob : this.zone.mobs) {
        if ( Util.getDistance(this, mob) <= 200) {
            mobs.add(mob);
            Logger.log("Tìm thấy quái gần đó: ID = " + mob.id + ", HP = " + mob.point.hp);
        }
        if (mobs.size() >= maxMobs) {
            Logger.log("Đã đạt giới hạn tối đa " + maxMobs + " quái lan sát thương.");
            break;
        }
    }
    if (mobs.isEmpty()) {
        Logger.log("Không tìm thấy quái nào gần Player ID = " + target.id);
    }
    return mobs;
}
    ...
    protected void setBom(Player plAtt) {
        ...
        long dame = Util.maxIntValue(this.nPoint.hpMax) / 2;
        SkillService.gI().sendPlayerPrepareBom(plAtt, 2000);
        if (!this.isBoss) {
            for (Mob mob : this.zone.mobs) {
                if (Util.getDistance(this, mob) <= rangeBom) { //khoảng cách có tác dụng bom
        ...
        if (!MapService.gI().isMapOffline(this.zone.map.mapId)) {
            for (Player pl : playersMap) {
                if (!this.equals(pl) && SkillService.gI().canAttackPlayer(this, pl) && Util.getDistance(this, pl) <= rangeBom) {
                    dame = pl.isBoss ? this.effectSkill.isMonkey ? dame / 3 : dame / 2 : dame;
                    pl.injured(this, dame, MapService.gI().isMapYardart(this.zone.map.mapId), false);
                    PlayerService.gI().sendInfoHpMpMoney(pl);
                    Service.gI().Send_Info_NV(pl);
                }
            }
        }
        Service.gI().chat(this, "Bùm Bùm");
    }
