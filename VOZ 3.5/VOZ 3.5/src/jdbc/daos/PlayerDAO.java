...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.clear();
            String moc_nap = dataArray.toJSONString();
            dataArray.clear();
            dataArray.add(gender == 0 ? 0 : gender == 1 ? 2 : 4);
            String dataBoughtSkill = dataArray.toJSONString();
            dataArray.clear();
            JSONObject Timeon1 = new JSONObject();
            Timeon1.put("Timeonline", 0);
            Timeon1.put("mocdanhan", 0);
            Timeon1.put("timers", 0);
            String Timeon = Timeon1.toJSONString();
            dataArray.clear();
            DBConnecter.executeUpdate("insert into player "
                    ...
                    + "data_black_ball, data_vip, data_side_task, dataBadges, dataTaskBadges, "
                    + "dailyGift, Achievement, Achievement_SucManh, BoughtSkill, TimeOn, moc_nap) "
                    + "values ()",
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataVip, dataSideTask, dataBadges, dataTaskBadges, dailyGift,
                     Archievement, ArchievementSucManh, dataBoughtSkill, Timeon, moc_nap);
...
//data vip
                ...
                dataArray.clear();
                JSONObject timeon1 = new JSONObject();
                timeon1.put("Timeonline", player.TimeOnline);
                timeon1.put("mocdanhan", player.mocquaonline);
                timeon1.put("timers", player.timereset);
                String TimeOn = timeon1.toJSONString();
                dataArray.clear();
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "hp_point_fusion = ?, mp_point_fusion = ?, dame_point_fusion = ? , BoughtSkill = ?, LearnSkill = ?, TimeOn = ? where id = ?";
